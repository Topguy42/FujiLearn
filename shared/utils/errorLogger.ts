...
export function logError(
  ...
  try {
    const savedErrors = localStorage.getItem('fujilearn_error_logs');
    ...
    localStorage.setItem('fujilearn_error_logs', JSON.stringify(trimmedLogs));
  } catch (storageError) {
...
export function clearErrorLogs(): void {
  try {
    localStorage.removeItem('fujilearn_error_logs');
  } catch (error) {
